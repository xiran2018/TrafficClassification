...
def load_index_labels(path: Path, label_map: dict[str, int]) -> np.ndarray:
    labels: list[int] = []
    with path.open("r", encoding="utf-8") as handle:
...
def load_prediction(name: str, path: Path, expected_y_true: np.ndarray) -> dict[str, Any]:
...
def build_report(
    ...
    index_y_true = load_index_labels(sample_index, label_map)
    ...
    rows = [load_prediction(name, path.expanduser().resolve(), index_y_true) for name, path in inputs]
    y_true = index_y_true
    ...
    if consensus is not None:
        consensus_row = load_prediction(
            consensus[0], consensus[1].expanduser().resolve(), index_y_true
        )
